...
CREATE TABLE `monitor_server` (
  ...
  `ssh_password` varchar(512) DEFAULT NULL COMMENT 'SSH密码密文',
  `agent_path` varchar(256) DEFAULT NULL COMMENT 'Agent路径（历史兼容字段）',
  `agent_token` varchar(128) DEFAULT NULL COMMENT 'Agent上报Token（历史兼容字段）',
  `agent_enabled` char(1) NOT NULL DEFAULT '1' COMMENT 'Agent状态（历史兼容字段）',
  `remark` varchar(512) DEFAULT NULL COMMENT '备注',
  `del_flag` char(1) NOT NULL DEFAULT '0' COMMENT '删除标志（0存在 1删除）',
  `create_by` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
