...
如果需要重新导入初始化 SQL，可以先停止服务，再清理 MySQL 和 Redis 运行数据。
...
Remove-Item -Recurse -Force .\docker\redis\data\*
docker compose up -d --build
```
...
```bash
docker compose down
rm -rf docker/mysql/data/* docker/redis/data/*
docker compose up -d --build
```

注意：清理 `docker/mysql/data/` 会删除当前数据库数据，执行前请确认不需要保留旧数据。
